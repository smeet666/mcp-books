...
1. Bump the version in **five** places, which must stay in step:
   ...
   - `src/version.ts`, which feeds the User-Agent sent to each site
...
```bash
npm view mcp-books version
npx -y mcp-books          # should start and wait on stdin
```
...
It matters here because the upstream is a pair of public archives rather than a
versioned API: the response shapes this server reads can change without notice,
and the unit tests run on invented fixtures that would stay green while the
published server is broken for everyone.

Two of its assertions are worth knowing about before a release. One holds the
full-text index that publishes no leaf number to publishing none, and the other
holds a matched passage apart from the opening of a page. Both are claims every
answer this server gives is built on, and neither can be checked without calling
the archives.
