...
`mcp-books` is a read-only client for [the Internet Archive, the Library of Congress and data.bnf.fr](https://archive.org). It runs on your
...
**3 hosts are contacted**, and nothing else.

| Host          | What is read there                           |
| ------------- | -------------------------------------------- |
| `archive.org` | the Archive's catalogue and its scanned text |
| `www.loc.gov` | the Library's catalogue                      |
| `data.bnf.fr` | the open catalogue's SPARQL endpoint         |
...
| A `User-Agent`         | `mcp-books/<version> (+https://github.com/smeet666/mcp-books)`, so the site can reach a person about the traffic it receives. |
| Your IP address        | Sent by your network to any host you contact, as with any web request.                                                        |

Your requests reach the Internet Archive, the Library of Congress and data.bnf.fr. What is done with them there is governed by each site's own privacy policy, which this project does not control.
...
becomes of them. `BOOKS_LOG_LEVEL` governs how much is written and defaults to `error`. These lines stay on your machine.

## The settings that change any of this

| Variable             | What it changes                                                                                                  |
| -------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `BOOKS_USER_AGENT`   | Adds your own identifier in front of this project's, which stays appended so the site can always reach a person. |
| `BOOKS_CACHE_TTL_MS` | How long an answer is held in memory. `0` turns the cache off.                                                   |
| `BOOKS_LOG_LEVEL`    | How much is written to stderr.                                                                                   |
...
Open an issue on [the repository](https://github.com/smeet666/mcp-books/issues). For something exploitable,
...
`mcp-books` est un client en lecture seule pour [l'Internet Archive, la Library of Congress et data.bnf.fr](https://archive.org). Il
...
**3 hôtes sont joints**, et rien d'autre.

| Hôte          | Ce qui y est lu                                 |
| ------------- | ----------------------------------------------- |
| `archive.org` | le catalogue de l'Archive et son texte numérisé |
| `www.loc.gov` | le catalogue de la Bibliothèque                 |
| `data.bnf.fr` | le point SPARQL du catalogue ouvert             |
...
| Un `User-Agent`   | `mcp-books/<version> (+https://github.com/smeet666/mcp-books)`, pour que le site puisse joindre une personne au sujet du trafic qu'il reçoit. |
| Votre adresse IP  | Transmise par votre réseau à tout hôte que vous joignez, comme pour n'importe quelle requête web.                                             |

Vos requêtes atteignent l'Internet Archive, la Library of Congress et data.bnf.fr. Ce qui en est fait là-bas relève de la politique de confidentialité propre à chaque site, que ce projet ne contrôle pas.
...
qu'ils deviennent. `BOOKS_LOG_LEVEL` règle leur quantité et vaut `error` par défaut. Ces lignes restent sur votre machine.

## Les réglages qui changent tout cela

| Variable             | Ce qu'elle change                                                                                                        |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `BOOKS_USER_AGENT`   | Ajoute votre identifiant devant celui du projet, qui reste accolé pour que le site puisse toujours joindre une personne. |
| `BOOKS_CACHE_TTL_MS` | Combien de temps une réponse est gardée en mémoire. `0` éteint le cache.                                                 |
| `BOOKS_LOG_LEVEL`    | La quantité écrite sur stderr.                                                                                           |
...
Ouvrez une issue sur [le dépôt](https://github.com/smeet666/mcp-books/issues). Pour quelque chose
