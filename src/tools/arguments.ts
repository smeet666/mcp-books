...
/**
 * How many unknown names a refusal repeats.
 *
 * The names come from the caller, and a request carrying ten thousand of them
 * would otherwise come back as a ten-thousand-entry sentence.
 */
const MOST_NAMED = 5;

/** Past this length a name is a payload rather than a name, and is not matched. */
const LONGEST_NAME = 64;

function unknownArgumentMessage(keys: readonly string[], declared: readonly string[]): string {
  const shown = keys.slice(0, MOST_NAMED);
  const named = shown
    .map((key) => {
      const shortened = key.length > LONGEST_NAME ? `${key.slice(0, LONGEST_NAME)}…` : key;
      const near = nearestArgument(key, declared);
      return near ? `'${shortened}' (did you mean '${near}'?)` : `'${shortened}'`;
    })
    .join(", ");
...
/** One edit is one character, so a two-character slip sits twice as far away. */
